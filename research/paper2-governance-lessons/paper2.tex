...
\begin{abstract}
Online platforms such as Wikipedia, Reddit, and Stack Overflow have spent two decades developing governance mechanisms for collaborative knowledge curation: defenses against Sybil attacks, coordinated manipulation, reputation gaming, consensus failures, and content degradation. As AI agents begin participating in shared knowledge bases, they face many of the same structural challenges, while the agent context introduces qualitatively new failure modes: sycophancy-driven consensus collapse, where agents agree for social harmony rather than accuracy; cascading hallucinations, where false claims could propagate through internal citation chains; training data homogenization, where models trained on each other's outputs lose perspective diversity before curation begins; and adaptive manipulation campaigns capable of adjusting strategy in real time. We conduct a structured literature review screening 160 papers across platform governance, trust systems, and multi-agent collaboration, and identify which platform defenses transfer to the agent setting, which break down, and which gaps require entirely new mechanisms. From this analysis, we propose nine design considerations for agent knowledge curation, grounded in platform evidence and adapted for agent-specific failure modes. We also identify critical gaps that platform-derived mechanisms cannot address: hallucination debt that accumulates when governance evaluates conduct but not content, onboarding hostility that punishes well-intentioned newcomers, and the absence of mechanisms to distinguish technical failure from adversarial behavior. We illustrate several of these considerations with AIngram, a prototype agent-native knowledge base that implements reputation decay, graduated sanctions, and vote audit trails. To our knowledge, no existing survey bridges platform governance research to multi-agent knowledge curation; we identify concrete open problems for formal protocol design and adversarial evaluation.
\end{abstract}
...
While game theory, mechanism design, and blockchain-inspired protocols offer valuable tools for agent governance, they risk overlooking two decades of empirical lessons from platform governance. Our approach builds on this foundation, adapting proven mechanisms to the agent setting while identifying where new solutions are needed.

This paper makes three contributions. First, we conduct a structured review that maps platform governance challenges to the agent knowledge curation setting, organized by problem type rather than by platform. Second, we propose nine design considerations for agent curation, each derived from specific platform evidence and adapted for agent-specific failure modes. These considerations are not individually novel; the contribution is the identification of which platform mechanisms transfer, which require modification, and what gaps remain. Third, we identify open problems that require formal treatment, establishing the research agenda for subsequent work on protocol formalization and adversarial evaluation.
...
\textbf{Methodology.} We conducted a structured literature review across Semantic Scholar, OpenAlex, arXiv, Google Scholar, and DBLP, screening 160 papers across thematic areas spanning platform governance, trust and reputation systems, multi-agent debate, content moderation, and adversarial knowledge manipulation. Of these, 114 were excluded as insufficiently specific to knowledge curation governance or redundant with higher-impact work in the same area. We selected 45 references based on four criteria: direct relevance to knowledge curation governance, coverage of competing and alternative approaches, empirical evidence supporting specific design decisions, and recency of agent-specific work. We performed backward citation chaining on seven high-impact papers (Du et al.~\cite{du2023}, Vosoughi et al.~\cite{vosoughi2018}, Zhang et al.~\cite{zhang2024a}, Shumailov et al.~\cite{shumailov2024}, Yao et al.~\cite{yao2025}, and Pitre et al.~\cite{pitre2025}) to verify coverage, and searched explicitly for existing surveys bridging platform governance to multi-agent systems, finding none.
...
6. Dissent protection & ArbCom incumbency bias \cite{cambridge2023}, model collapse \cite{shumailov2024} & Weighted dissent bonus in reputation scoring, retaliation detection \\
...
8. Conduct over content & ArbCom behavior-only rulings \cite{cambridge2023} & Protocol compliance scoring (source manufacturing risk acknowledged) \\
...
\bibitem{cambridge2023} Cambridge University Press. Wikipedia Arbitration Committee analysis. 2023.
