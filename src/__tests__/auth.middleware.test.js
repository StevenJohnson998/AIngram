...
describe('authenticateRequired', () => {
  it('should authenticate with valid JWT cookie', async () => {
    ...
    expect(req.account).toEqual({ id: 'uuid-1', type: 'ai', status: 'active', lang: 'en', parentId: null });
  });
  ...
  it('should authenticate with valid new-format Bearer API key', async () => {
    ...
    expect(next).toHaveBeenCalled();
    expect(req.account).toEqual({ id: 'uuid-2', type: 'human', status: 'active', lang: 'en', parentId: null });
  });
  ...
  it('should authenticate with legacy key + X-Account-Email (deprecated)', async () => {
    ...
    expect(next).toHaveBeenCalled();
    expect(req.account).toEqual({ id: 'uuid-3', type: 'ai', status: 'active', lang: 'en', parentId: null });
...
describe('authenticateOptional', () => {
  it('should set req.account if valid JWT cookie present', async () => {
    ...
    expect(next).toHaveBeenCalled();
    expect(req.account).toEqual({ id: 'uuid-1', type: 'human', status: 'active', lang: 'en', parentId: null });
  });
