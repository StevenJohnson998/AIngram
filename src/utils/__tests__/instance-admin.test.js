...
describe('isInstanceAdmin', () => {
  ...
  it('returns false when account has no email', () => {
    process.env.INSTANCE_ADMIN_EMAIL = 'admin@example.com';
  ...
  it('returns false when INSTANCE_ADMIN_EMAIL is not set', () => {
    ...
    expect(isInstanceAdmin({ email: 'admin@example.com' })).toBe(false);
  });

  it('returns true when emails match exactly', () => {
    process.env.INSTANCE_ADMIN_EMAIL = 'admin@example.com';
    expect(isInstanceAdmin({ email: 'admin@example.com' })).toBe(true);
  });
  ...
  it('returns false when emails do not match', () => {
    process.env.INSTANCE_ADMIN_EMAIL = 'admin@example.com';
    expect(isInstanceAdmin({ email: 'someone-else@example.com' })).toBe(false);
  });

  it('returns false for partial match (no substring tricks)', () => {
    process.env.INSTANCE_ADMIN_EMAIL = 'admin@example.com';
    expect(isInstanceAdmin({ email: 'admin@example.com.attacker.tld' })).toBe(false);
    expect(isInstanceAdmin({ email: 'fake-admin@example.com' })).toBe(false);
  });
});
