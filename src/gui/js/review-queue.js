...
var currentPage = 1;
    ...
    document.addEventListener('DOMContentLoaded', async function() {
      ...
      if (!user) {
        ...
        document.getElementById('review-error').innerHTML = '<div class="alert alert-warning">' + t('You must be') + ' <a href="./login.html">' + t('logged in') + '</a> ' + t('with review permissions to access the review queue.') + '</div>';
        return;
      }
    ...
    async function loadFlags(status, page, append) {
      ...
      try {
        ...
        if (res.status === 403) {
          document.getElementById('flags-container').innerHTML = '<div class="alert alert-warning">' + t('Review access requires building up your reputation first. Keep contributing to earn it!') + '</div>';
          return;
        }
        ...
        if (res.status === 200 && res.data && res.data) {
          ...
          document.getElementById('flag-count').textContent = (total !== 1 ? t('{n} flags {status}', { n: total, status: t(status) }) : t('{n} flag {status}', { n: total, status: t(status) }));

          if (flags.length === 0 && !append) {
            document.getElementById('flags-container').innerHTML = '<p class="text-muted">' + t('No flags with status "{status}".', { status: t(status) }) + '</p>';
          ...
          var html = flags.map(function(flag) {
            ...
            if (flag.target_type === 'topic' && flag.target_id) {
              targetLink = '<a href="./topic.html?id=' + encodeURIComponent(flag.target_id) + '">' + t('View topic') + '</a>';
            } else if (flag.target_type === 'chunk' && flag.target_id) {
              targetLink = '<span>' + t('Chunk {id}', { id: escapeHtml(flag.target_id.substring(0, 8)) }) + '</span>';
            }
            return '<div class="review-item">' +
              ...
              '<div class="review-body">' +
                ...
                '<div class="review-meta">' +
                  '<span>' + t('Target: {type}', { type: escapeHtml(t(flag.target_type)) }) + '</span>' +
                  (targetLink ? '<span class="sep">&middot;</span>' + targetLink : '') +
                  '<span class="sep">&middot;</span>' +
                  '<span>' + t('Status: {status}', { status: escapeHtml(t(flag.status)) }) + '</span>' +
                  '<span class="sep">&middot;</span>' +
                  '<span>' + timeAgo(flag.created_at) + '</span>' +
                '</div>' +
                '<div class="review-tags">' +
                  '<span class="badge badge-trust-medium">' + escapeHtml(t(flag.status)) + '</span>' +
                '</div>' +
              '</div>' +
              '<div class="review-actions">' +
                (flag.status === 'open' ? '<button class="btn btn-secondary btn-sm flag-review-btn" data-id="' + flag.id + '">' + t('Review') + '</button>' : '') +
                (flag.status === 'open' || flag.status === 'reviewing' ? '<button class="btn btn-secondary btn-sm flag-dismiss-btn" data-id="' + flag.id + '">' + t('Dismiss') + '</button>' : '') +
                (flag.status === 'open' || flag.status === 'reviewing' ? '<button class="btn btn-danger btn-sm flag-action-btn" data-id="' + flag.id + '">' + t('Action') + '</button>' : '') +
              '</div>' +
            '</div>';
          }).join('');
        ...
        document.getElementById('flags-container').innerHTML = '<div class="alert alert-warning">' + t('Failed to load flags.') + '</div>';
      }
    }
    ...
    async function flagAction(flagId, action) {
      try {
        ...
        if (res.status === 200) {
          ...
          var msg = (res.data && res.data.error) ? res.data.error.message : t('Action failed');
          alert(msg);
        }
      } catch (err) {
    ...
    async function loadProposals() {
      ...
      try {
        ...
        if (res.status === 403) {
          container.innerHTML = '<div class="alert alert-warning">' + t('Review access required. Keep contributing to earn this permission.') + '</div>';
          return;
        }

        if (res.status !== 200 || !res.data || !res.data.items) {
          container.innerHTML = '<p class="text-muted">' + t('No proposed edits.') + '</p>';
          return;
        }

        var proposals = res.data.items;
        if (proposals.length === 0) {
          container.innerHTML = '<p class="text-muted">' + t('No proposed edits pending.') + '</p>';
          return;
        }
        ...
        container.querySelectorAll('.proposal-merge-btn').forEach(function(btn) {
          btn.addEventListener('click', async function() {
            ...
            try {
              ...
              else alert((res.data && res.data.error) ? res.data.error.message : t('Merge failed'));
            } catch (err) { alert(t('Network error.')); }
            b.disabled = false;
          });
        });
    ...
    function renderProposal(p) {
      var proposer = p.proposedByName ? escapeHtml(p.proposedByName) : t('Unknown');
      var diffHtml = '';
      ...
      if (p.original_content) {
        ...
        diffHtml = '<div class="diff-container">' +
          '<div class="diff-panel">' +
            '<div class="diff-panel-header">' + t('Original') + '</div>' +
            diffResult.left +
          '</div>' +
          '<div class="diff-panel">' +
            '<div class="diff-panel-header">' + t('Proposed') + '</div>' +
            diffResult.right +
          '</div>' +
        '</div>';
      } else {
        diffHtml = '<div class="diff-container">' +
          '<div class="diff-panel s-634a28be">' +
            '<div class="diff-panel-header">' + t('New chunk') + '</div>' +
      ...
      if (p.discussionMessageCount > 0) {
        discussionLink = '<span class="sep">&middot;</span><span title="' + t('Has discussion') + '">&#128172; ' + t('Discussion') + '</span>';
      }
      ...
      return '<div class="review-item">' +
        ...
        '<div class="review-body">' +
          ...
          '<div class="review-meta s-05460b51">' +
            ...
            '<span class="sep">&middot;</span>' +
            '<span>' + t('by {proposer}', { proposer: proposer }) + '</span>' +
            '<span class="sep">&middot;</span>' +
        ...
        '</div>' +
        '<div class="review-actions">' +
          '<button class="btn btn-primary btn-sm proposal-merge-btn" data-id="' + p.changesetId + '">' + t('Merge') + '</button>' +
          '<button class="btn btn-danger btn-sm proposal-reject-btn" data-id="' + p.changesetId + '">' + t('Reject') + '</button>' +
        '</div>' +
      '</div>';
    }
    ...
    async function confirmReject() {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          alert((res.data && res.data.error) ? res.data.error.message : t('Reject failed'));
        }
      } catch (err) {
        alert(t('Network error.'));
      }
      btn.disabled = false;
      btn.textContent = t('Reject');
    }
